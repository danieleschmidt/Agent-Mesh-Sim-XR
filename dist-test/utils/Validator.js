...
class Validator {
    ...
    static validateAgentUpdate(update) {
    ...
    static validateXRSessionConfig(config) {
        if (!config || typeof config !== 'object') {
    ...
    static sanitizeAgentData(agent) {
        ...
        return {
            ...
            activeGoals: agent.activeGoals.map((goal) => this.sanitizeString(String(goal), 200)),
            connectedPeers: agent.connectedPeers.map((peer) => this.sanitizeString(String(peer), 100))
        };
    }
    ...
    static isValidAgentState(state) {
        if (!state || typeof state !== 'object')
            return false;
        const validStatuses = ['active', 'idle', 'error', 'paused'];
        if (!validStatuses.includes(state.status))
            return false;
        if (typeof state.behavior !== 'string')
            return false;
        if (typeof state.role !== 'string')
            return false;
        if (typeof state.energy !== 'number' || state.energy < 0 || state.energy > 100)
            return false;
        if (typeof state.priority !== 'number' || state.priority < 1 || state.priority > 10)
            return false;
        return true;
    }
    static isValidMetrics(metrics) {
