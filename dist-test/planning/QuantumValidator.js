...
exports.QuantumValidator = exports.QuantumValidationError = void 0;
const three_1 = require("three");
class QuantumValidationError extends Error {
    field;
    value;
    constraints;
...
exports.QuantumValidationError = QuantumValidationError;
class QuantumValidator {
    ...
    constructor() {
        this.validationRules = {
            ...
            positionBounds: {
                min: new three_1.Vector3(-1000, -1000, -1000),
                max: new three_1.Vector3(1000, 1000, 1000)
            }
        };
    }
    static getInstance() {
    ...
    validateQuantumTask(task) {
        ...
        try {
            ...
            errors.push(new QuantumValidationError('Unexpected validation error', 'task', task, { error: error.message }));
    ...
    validateQuantumState(state) {
        ...
        try {
            ...
            if (typeof state.coherence !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Coherence must be between ${this.validationRules.coherenceRange.min} and ${this.validationRules.coherenceRange.max}`, 'coherence', state.coherence, this.validationRules.coherenceRange));
            }
            ...
            if (typeof state.interference !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Interference must be between -${this.validationRules.maxInterferenceStrength} and ${this.validationRules.maxInterferenceStrength}`, 'interference', state.interference, { max: this.validationRules.maxInterferenceStrength }));
            }
            ...
            errors.push(new QuantumValidationError('Unexpected quantum state validation error', 'quantumState', state, { error: error.message }));
            return { isValid: false, errors, warnings };
        }
    }
    // Validate superposition state
    validateSuperpositionState(state) {
        const errors = [];
        const warnings = [];
        try {
            // Validate ID
            if (!state.id || typeof state.id !== 'string') {
                errors.push(new QuantumValidationError('Superposition state ID must be a non-empty string', 'id', state.id, { required: true, type: 'string' }));
            }
            // Validate amplitude
            if (typeof state.amplitude !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Amplitude must be between ${this.validationRules.amplitudeRange.min} and ${this.validationRules.amplitudeRange.max}`, 'amplitude', state.amplitude, this.validationRules.amplitudeRange));
            }
            // Validate phase
            if (typeof state.phase !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Phase must be between ${this.validationRules.phaseRange.min} and ${this.validationRules.phaseRange.max}`, 'phase', state.phase, this.validationRules.phaseRange));
            }
            ...
            if (typeof state.energy !== 'number' || state.energy < 0) {
                errors.push(new QuantumValidationError('Energy must be a non-negative number', 'energy', state.energy, { min: 0 }));
            }
            return {
                isValid: errors.length === 0,
                errors,
            ...
            errors.push(new QuantumValidationError('Unexpected superposition state validation error', 'superpositionState', state, { error: error.message }));
            return { isValid: false, errors, warnings };
        }
    }
    // Validate quantum system
    validateQuantumSystem(system) {
        const errors = [];
        const warnings = [];
        try {
            // Validate system ID
            if (!system.id || typeof system.id !== 'string') {
                errors.push(new QuantumValidationError('Quantum system ID must be a non-empty string', 'id', system.id, { required: true, type: 'string' }));
            }
            // Validate states
            if (!system.states || system.states.size === 0) {
                errors.push(new QuantumValidationError('Quantum system must have at least one state', 'states', system.states, { minStates: 1 }));
            }
            else if (system.states.size > this.validationRules.maxSuperpositionStates) {
                errors.push(new QuantumValidationError(`Quantum system cannot have more than ${this.validationRules.maxSuperpositionStates} states`, 'states', system.states.size, { maxStates: this.validationRules.maxSuperpositionStates }));
            }
            ...
            if (Math.abs(totalProbability - 1.0) > this.validationRules.probabilityTolerance) {
                errors.push(new QuantumValidationError(`Total probability must equal 1.0, got ${totalProbability}`, 'states', totalProbability, { expected: 1.0, tolerance: this.validationRules.probabilityTolerance }));
            }
            // Validate entanglements
            if (system.entangled.length > this.validationRules.maxEntanglements) {
                errors.push(new QuantumValidationError(`System cannot have more than ${this.validationRules.maxEntanglements} entanglements`, 'entangled', system.entangled.length, { maxEntanglements: this.validationRules.maxEntanglements }));
            }
            // Validate coherence time
            if (typeof system.coherenceTime !== 'number' || system.coherenceTime <= 0) {
                errors.push(new QuantumValidationError('Coherence time must be a positive number', 'coherenceTime', system.coherenceTime, { min: 0, exclusive: true }));
            }
            // Validate decoherence rate
            if (typeof system.decoherenceRate !== 'number' || system.decoherenceRate < 0 || system.decoherenceRate > 1) {
                errors.push(new QuantumValidationError('Decoherence rate must be between 0 and 1', 'decoherenceRate', system.decoherenceRate, { min: 0, max: 1 }));
            }
            return {
                isValid: errors.length === 0,
                errors,
            ...
            errors.push(new QuantumValidationError('Unexpected quantum system validation error', 'quantumSystem', system, { error: error.message }));
            return { isValid: false, errors, warnings };
        }
    }
    // Validate interference pattern
    validateInterferencePattern(pattern) {
        const errors = [];
        const warnings = [];
        try {
            // Validate ID
            if (!pattern.id || typeof pattern.id !== 'string') {
                errors.push(new QuantumValidationError('Interference pattern ID must be a non-empty string', 'id', pattern.id, { required: true, type: 'string' }));
            }
            // Validate type
            const validTypes = ['constructive', 'destructive', 'mixed'];
            if (!validTypes.includes(pattern.type)) {
                errors.push(new QuantumValidationError(`Interference type must be one of: ${validTypes.join(', ')}`, 'type', pattern.type, { validTypes }));
            }
            // Validate strength
            if (typeof pattern.strength !== 'number' ||
                pattern.strength < 0 ||
                pattern.strength > this.validationRules.maxInterferenceStrength) {
                errors.push(new QuantumValidationError(`Interference strength must be between 0 and ${this.validationRules.maxInterferenceStrength}`, 'strength', pattern.strength, { min: 0, max: this.validationRules.maxInterferenceStrength }));
            }
            // Validate frequency
            if (typeof pattern.frequency !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Frequency must be between ${this.validationRules.frequencyRange.min} and ${this.validationRules.frequencyRange.max}`, 'frequency', pattern.frequency, this.validationRules.frequencyRange));
            }
            // Validate phase
            if (typeof pattern.phase !== 'number' ||
                ...
                errors.push(new QuantumValidationError(`Phase must be between ${this.validationRules.phaseRange.min} and ${this.validationRules.phaseRange.max}`, 'phase', pattern.phase, this.validationRules.phaseRange));
            }
            // Validate ranges
            if (typeof pattern.spatialRange !== 'number' || pattern.spatialRange <= 0) {
                errors.push(new QuantumValidationError('Spatial range must be a positive number', 'spatialRange', pattern.spatialRange, { min: 0, exclusive: true }));
            }
            if (typeof pattern.temporalRange !== 'number' || pattern.temporalRange <= 0) {
                errors.push(new QuantumValidationError('Temporal range must be a positive number', 'temporalRange', pattern.temporalRange, { min: 0, exclusive: true }));
            }
            return {
                isValid: errors.length === 0,
                errors,
            ...
            errors.push(new QuantumValidationError('Unexpected interference pattern validation error', 'interferencePattern', pattern, { error: error.message }));
            return { isValid: false, errors, warnings };
        }
    }
    ...
    sanitizeSuperpositionState(state) {
        return {
            ...state,
    ...
    sanitizeQuantumSystem(system) {
        const sanitizedStates = new Map();
    ...
    sanitizeInterferencePattern(pattern) {
        return {
            ...pattern,
    ...
    validateTaskId(id, errors) {
        if (!id || typeof id !== 'string' || id.trim().length === 0) {
            errors.push(new QuantumValidationError('Task ID must be a non-empty string', 'id', id, { required: true, type: 'string' }));
        }
    }
    validateTaskDescription(description, errors, warnings) {
        if (!description || typeof description !== 'string') {
            errors.push(new QuantumValidationError('Task description must be a string', 'description', description, { required: true, type: 'string' }));
        }
        else if (description.length > 1000) {
            warnings.push('Task description is very long and may impact performance');
        }
    }
    validateTaskPriority(priority, errors) {
        if (typeof priority !== 'number' ||
            priority < this.validationRules.taskPriorityRange.min ||
            priority > this.validationRules.taskPriorityRange.max) {
            errors.push(new QuantumValidationError(`Task priority must be between ${this.validationRules.taskPriorityRange.min} and ${this.validationRules.taskPriorityRange.max}`, 'priority', priority, this.validationRules.taskPriorityRange));
        }
    }
    validateTaskDuration(duration, errors) {
        if (typeof duration !== 'number' || duration <= 0) {
            errors.push(new QuantumValidationError('Task duration must be a positive number', 'estimatedDuration', duration, { min: 0, exclusive: true }));
        }
    }
    validateRequiredAgents(agents, errors) {
        if (typeof agents !== 'number' ||
            agents < this.validationRules.agentLimits.min ||
            agents > this.validationRules.agentLimits.max) {
            errors.push(new QuantumValidationError(`Required agents must be between ${this.validationRules.agentLimits.min} and ${this.validationRules.agentLimits.max}`, 'requiredAgents', agents, this.validationRules.agentLimits));
        }
    }
    validateTaskDependencies(dependencies, errors) {
        if (!Array.isArray(dependencies)) {
            errors.push(new QuantumValidationError('Task dependencies must be an array', 'dependencies', dependencies, { type: 'array' }));
        }
        else {
            dependencies.forEach((dep, index) => {
                if (!dep || typeof dep !== 'string') {
                    errors.push(new QuantumValidationError(`Dependency at index ${index} must be a non-empty string`, `dependencies[${index}]`, dep, { required: true, type: 'string' }));
                }
            });
        }
    }
    validateTaskPosition(position, errors, warnings) {
        if (position) {
            if (!(position instanceof three_1.Vector3)) {
                errors.push(new QuantumValidationError('Task position must be a Vector3 instance', 'position', position, { type: 'Vector3' }));
            }
            else {
    ...
    validateTaskConstraints(constraints, errors) {
        if (!Array.isArray(constraints)) {
            errors.push(new QuantumValidationError('Task constraints must be an array', 'constraints', constraints, { type: 'array' }));
        }
        else {
            constraints.forEach((constraint, index) => {
                if (!constraint || typeof constraint !== 'object') {
                    errors.push(new QuantumValidationError(`Constraint at index ${index} must be an object`, `constraints[${index}]`, constraint, { type: 'object' }));
                }
            });
        }
    }
    validateSuperposition(superposition, errors, warnings) {
        if (!(superposition instanceof Map)) {
            errors.push(new QuantumValidationError('Superposition must be a Map', 'superposition', superposition, { type: 'Map' }));
            return;
        }
        if (superposition.size === 0) {
            errors.push(new QuantumValidationError('Superposition must have at least one state', 'superposition', superposition.size, { minStates: 1 }));
        }
        if (superposition.size > this.validationRules.maxSuperpositionStates) {
            errors.push(new QuantumValidationError(`Superposition cannot have more than ${this.validationRules.maxSuperpositionStates} states`, 'superposition', superposition.size, { maxStates: this.validationRules.maxSuperpositionStates }));
        }
    ...
    validateEntanglements(entangled, errors, warnings) {
        if (!Array.isArray(entangled)) {
            errors.push(new QuantumValidationError('Entangled systems must be an array', 'entangled', entangled, { type: 'array' }));
        }
    ...
    sanitizePosition(position) {
        if (!position || !(position instanceof three_1.Vector3))
            return position;
        const bounds = this.validationRules.positionBounds;
        return new three_1.Vector3(this.clamp(position.x, bounds.min.x, bounds.max.x), this.clamp(position.y, bounds.min.y, bounds.max.y), this.clamp(position.z, bounds.min.z, bounds.max.z));
    }
    sanitizeConstraint(constraint) {
        return {
            ...constraint,
            type: this.sanitizeString(constraint.type),
