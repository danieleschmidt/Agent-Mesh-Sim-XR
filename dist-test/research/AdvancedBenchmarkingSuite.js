...
exports.AdvancedBenchmarkingSuite = void 0;
 ...
 */
class AdvancedBenchmarkingSuite extends eventemitter3_1.EventEmitter {
    performanceMonitor;
    benchmarkResults = [];
    ...
    constructor(performanceMonitor) {
        ...
        Logger_1.logger.info('AdvancedBenchmarkingSuite', 'Initialized with environment', {
    ...
    initializeStandardMetrics() {
        ...
        this.registerMetric({
            ...
            measure: async (context) => {
                const report = context.performanceMonitor.getPerformanceReport();
                return report.fps || 0;
            },
            higherIsBetter: true,
    ...
    registerMetric(metric) {
        ...
        Logger_1.logger.debug('AdvancedBenchmarkingSuite', 'Metric registered', { name: metric.name });
    }
    async runBenchmark(config) {
        Logger_1.logger.info('AdvancedBenchmarkingSuite', 'Starting benchmark suite', {
            name: config.name,
        ...
        for (const scenario of config.scenarios) {
            for (const agentCount of config.agentCounts) {
                try {
                    Logger_1.logger.info('AdvancedBenchmarkingSuite', `Running scenario: ${scenario.name} with ${agentCount} agents`);
                    ...
                    Logger_1.logger.error('AdvancedBenchmarkingSuite', `Scenario failed: ${scenario.name}`, error);
                    const failedResult = {
        ...
        Logger_1.logger.info('AdvancedBenchmarkingSuite', 'Benchmark suite completed', {
            totalResults: results.length,
            successfulResults: results.filter(r => Object.keys(r.metrics).length > 0).length
        });
        return results;
    }
    async runScenario(config, scenario, agentCount) {
        ...
        for (const metricName of config.metrics.map(m => m.name)) {
            ...
            if (metric) {
                try {
                    ...
                    Logger_1.logger.warn('AdvancedBenchmarkingSuite', `Metric measurement failed: ${metricName}`, error);
    ...
    createPerformanceProfile(context) {
        const report = context.performanceMonitor.getPerformanceReport();
        return {
            avgFPS: report.fps || 0,
    ...
    detectAnomalies(profile, metrics) {
        const anomalies = [];
        // FPS drop detection
    ...
    async calculateStatisticalSignificance(config, scenarioName, agentCount, metrics) {
        // Find previous results for the same scenario and agent count
        const previousResults = this.benchmarkResults.filter(r => r.configName === config.name &&
        ...
        for (const [metricName, value] of Object.entries(metrics)) {
            const metric = this.activeMetrics.get(metricName);
            if (!metric?.significanceThreshold)
                continue;
            const previousValues = previousResults.map(r => r.metrics[metricName]).filter(v => v !== undefined);
            if (previousValues.length >= 3) {
                const isSignificant = this.statisticalAnalyzer.performTTest(previousValues, [value], 0.05 // 95% confidence level
                );
                if (isSignificant) {
                    return true;
                }
    ...
    async performStatisticalAnalysis(config, results) {
        return this.statisticalAnalyzer.analyzeResults(results);
    }
    generateBenchmarkReport(config, results, analysis) {
        const report = {
            configName: config.name,
    ...
    async publishResults(config, results, report) {
        // Simulate publishing results to research database
        Logger_1.logger.info('AdvancedBenchmarkingSuite', 'Publishing benchmark results', {
            configName: config.name,
    ...
    clearResults() {
        ...
        Logger_1.logger.info('AdvancedBenchmarkingSuite', 'Benchmark results cleared');
    }
    dispose() {
        this.clearResults();
...
exports.AdvancedBenchmarkingSuite = AdvancedBenchmarkingSuite;
